 	...
 	<div class="table-responsive">
 		<table class="table table-striped">
 			<form action="fungsi/edit/edit.php?barang=edit" method="POST">
 				...
 				<tr>
 					<td>Harga Jual</td>
 					<td><input type="number" class="form-control" id="harga_jual" value="<?php echo $hasil['harga_jual']; ?>" name="jual"></td>
 				</tr>
 				<tr>
 					<td>Diskon</td>
 					<td><input type="number" class="form-control" id="diskon" value="<?php echo $hasil['diskon']; ?>" name="diskon"></td>
 				</tr>
 				<tr>
 					<td>Final Price</td>
 					<td><input type="text" placeholder="0" required class="form-control" name="final" id="final" readonly></td>
 				</tr>
 				<tr>
 					<td>Satuan Barang</td>
 					<td>
 ...
 <script>
 	document.addEventListener("DOMContentLoaded", function() {
 		const jualInput = document.getElementById('harga_jual');
 		const diskonInput = document.getElementById('diskon');
 		const finalInput = document.getElementById('final');

 		function hitungHargaFinal() {
 			let harga = parseFloat(jualInput.value) || 0;
 			let diskon = parseFloat(diskonInput.value) || 0;

 			let totalHarga = harga - (harga * diskon / 100);
 			finalInput.value = totalHarga;
 		}

 		jualInput.addEventListener('input', hitungHargaFinal);
 		diskonInput.addEventListener('input', hitungHargaFinal);
 	});
 </script>